...
Backbone.ModelBinder=Backbone.Model.extend({_model:undefined,_rootEl:undefined,_attributeBindings:undefined,bind:function(model,rootEl,attributeBindings){this.unbind();this._model=model;this._rootEl=rootEl;if(!this._model)throw'model must be specified';if(!this._rootEl)throw'rootEl must be specified';if(attributeBindings){this._attributeBindings=$.extend(true,{},attributeBindings);}
    ...
    break;default:el.attr(elAttribute,convertedValue);}},_setElValue:function(el,convertedValue){if(el.attr('type')){switch(el.attr('type')){case'radio':if(el.attr('value')===convertedValue){el.attr('checked','checked');}
    ...
    data[elementBinding.attributeBinding.attributeName]=this._getConvertedValue(Backbone.ModelBinder.Constants.ViewToModel,elementBinding,elVal);this._model.set(data);},_getConvertedValue:function(direction,elementBinding,value){if(elementBinding.converter){value=elementBinding.converter(direction,value);}
    return value;}});Backbone.ModelBinder.Constants={};Backbone.ModelBinder.Constants.ModelToView='ModelToView';Backbone.ModelBinder.Constants.ViewToModel='ViewToModel';Backbone.ModelBinder.CollectionConverter=Backbone.Model.extend({collection:undefined,constructor:function(collection,formatter){this.collection=collection;if(!this.collection){throw'Collection must be defined';}
...
else{return this.collection.get(value);}}});